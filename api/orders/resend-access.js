...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Une relivraison fait aussi office de révocation du lien précédent.
    order.accessToken = makeAccessToken(state.profile.slug);
    try {
      const email = await sendAccessEmail({
        ...
        orderId: order.id,
        deliveryKey: `resend-${order.accessToken.slice(-12)}`,
      });
