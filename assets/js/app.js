...
const seedState = {
  ...
  analytics: {
    ...
    checkouts: 146,
    purchases: 77,
    sourceCounts: { Instagram: 2210, YouTube: 1160, Email: 870, Direct: 592, Other: 0 },
...
function normalizeState(input = {}) {
  ...
  next.analytics.sourceCounts = {
    ...base.analytics.sourceCounts,
...
function pagePublicPath(page) {
  const storeSlug = slugify(state.profile.slug || state.profile.creatorName || "boutique");
  return `/p/${storeSlug}/${page.slug}`;
}
...
function setView(view) {
  ...
  if (view === "emails") {
    ...
    showToast("La fonctionnalité Emails sera bientôt disponible.");
    return;
  }
  activeView = view;
...
function renderOverview() {
  ...
  const clickRate = state.analytics.visits
    ? ((Number(state.analytics.clicks || 0) / state.analytics.visits) * 100).toFixed(1).replace(".", ",")
    : "0,0";
  ...
  document.querySelector("#overviewMetrics").innerHTML = [
    ...
    metricCard("Taux de clics", `${clickRate} %`, "◎", "Réel", ` ${Number(state.analytics.clicks || 0)} clics enregistrés`),
  ...
  svg.innerHTML = `
    <defs>
      <linearGradient id="revenueFill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#073bd9" stop-opacity=".18"/>
        <stop offset="100%" stop-color="#073bd9" stop-opacity="0"/>
      </linearGradient>
    </defs>
    ${grid}
    <polygon points="0,235 ${line} 760,235" fill="url(#revenueFill)"></polygon>
    <polyline points="${line}" fill="none" stroke="#073bd9" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"></polyline>
    ${points.map(([x, y], index) => index % 2 === 0 ? `<circle cx="${x}" cy="${y}" r="3.5" fill="#fff" stroke="#073bd9" stroke-width="2.5"></circle>` : "").join("")}
  `;
}
...
function renderPages() {
        ...
        return `
          <article class="sales-page-card">
            <div class="sales-page-card-head">
              ...
              <span class="status-badge">Publiée</span>
            </div>
  ...
  const selectedPage = state.pages[0];
...
function renderAnalytics() {
  ...
  document.querySelector("#analyticsMetrics").innerHTML = [
    ...
    metricCard("Taux de clics", `${clickRate} %`, "◎", "Réel", ` ${Number(state.analytics.clicks || 0)} clics`),
  ...
  if (state.products.length && !state.pages.length) insights.push(["Créer une page", "Tes produits existent, mais aucun lien public ne convertit encore.", "pages"]);
...
async function submitPage(event) {
  ...
  showToast(existing ? "Page mise à jour." : "Page créée avec son lien public.");
}
